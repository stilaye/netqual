...
from utils.mdns_helpers import MDNSHelper, MDNS_ADDR, MDNS_PORT
...
class TestServiceAdvertisement:
    ...
    def test_mdns_query_packet_format(self):
        ...
        assert len(query) > 12, (
            f"Query too short: {len(query)} bytes — header alone is 12 bytes"
        )
        assert query[0:2] == b"\x00\x00", (
            "mDNS transaction ID must be 0x0000 (RFC 6762 §6)"
        )
        assert (query[2] & 0x80) == 0, (
            "QR bit must be 0 for a query (bit 15 of flags word)"
        )
    ...
    def test_mdns_query_encodes_service_label(self, service, label):
        ...
        assert label in query, (
            f"Expected label {label!r} not found in query for '{service}'"
        )
    ...
    def test_mdns_multicast_send(self):
        ...
        try:
            ...
            assert bytes_sent > 0, (
                f"Expected > 0 bytes sent, got {bytes_sent}"
            )
...
class TestServiceDiscovery:
    ...
    def test_parse_invalid_short_packet(self):
        ...
        assert result["valid"] is False, (
            "2-byte packet must be rejected (header is 12 bytes)"
        )
...
class TestBLEAdvertisementFormat:
    ...
    def test_ble_payload_contains_apple_company_id(self):
        ...
        assert b"\x4c\x00" in payload, (
            "Apple Company ID (0x4C 0x00) missing from BLE advertisement"
        )
