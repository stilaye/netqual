...
class ConnectionHelper:
    ...
    def connect_with_retry(
        ...
        for attempt in range(self.retry_count):
            try:
                logger.debug(
                    f"Connection attempt {attempt + 1}/{self.retry_count} to {host}:{port}"
                )
...
def check_host_reachable(host: str, port: int = 443, timeout: int = 5) -> Dict[str, Any]:
    ...
    """
    result = {"reachable": False, "latency_ms": None, "error": None, "resolved_ip": None}
...
class SSLValidator:
    ...
    @staticmethod
    def verify_cipher_strength(
        host: str, port: int = 443, min_bits: int = 128
...
def send_http_request(
    host: str, path: str = "/", port: int = 80, timeout: int = 10
) -> Optional[str]:
    """
